...
for j in range(0, 10):  # 10 repeats experiment
    ...
    print(f"Game starts {'UCT + h_G 3.5' if first_player==1 else 'UCT + h_G 20'}")
    while not position.check_is_terminal(print_who_won=False):
        ...
        if position.get_actual_player() == first_player:
            engine_mcts = MCTS_with_heuristic_h_G(C=3.5, selection_type="UCT", steps=6, G=3.5)
        else:
        ...
        position.draw_board()
    end_game = time.time()
    print(f"{'MCTS + UCT + h_G heuristics 3.5' if position.winner==(first_player==1) else 'MCTS + UCT + h_G heuristics 20'} ({'Green' if position.winner==(first_player==1) else 'Blue'}) won after {position.moves_made} moves!")
    ...
    # saving to file
    with open('results/1_tournament_UCT_with_both_heuristics_experiment.csv', 'a') as f:
        f.write(f'{j};{seed_values[j]};{"UCT+h_G3.5" if first_player==1 else "UCT+h_G20"};{"UCT+h_G20" if first_player==1 else "UCT+h_G3.5"};{"UCT+h_G3.5" if position.winner==(first_player==1) else "UCT+h_G20"};{position.moves_made};{game_time}\n')
